...
# Comment out the other 'teams' variable and use this one when testing.
# teams = [jets, yotes]
...
teams.each do |team|
  # Team data should always be the first element of the 'teams' key.
  team_data = team['teams'][0]

  # Divisions and conferences can be found using the APIs id:
  d = Division.find_by(division_id: team_data['division']['id'])
  c = Conference.find_by(conference_id: team_data['conference']['id'])

  # To make the team belongs_to division and conference associations work
  # you'll need to remove your 'division' and 'conference' String columns
  # from the 'teams' table and replace them with FK references.
  ...
  t = Team.create(
    ...
    year: team_data['firstYearOfPlay']
  )

  # If teams aren't being created, check this:
  # puts t.errors.messages

  team_data['roster']['roster'].each do |player|
    p = Player.create(
      player_id: player['person']['id'],
      team: t,
      jersey_number: player['jerseyNumber'],
    ...
    # If players aren't being created, check this:
